...
void EQAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    ...
    juce::dsp::ProcessSpec spec; // Especificação
...
void EQAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    juce::dsp::ProcessContextReplacing<float> hightContext(rightBlock);
    //********************************************************************************
    leftChain.process(leftContext);
    rightChain.process(hightContext);
    //********************************************************************************
}
...
{
    // Paramentros de qualidade dos filtros
...
void EQAudioProcessor::updateHighCutFilters(const ChainSettings &chainSettings)
{
    ...
    auto& righttHighCut = leftChain.get<ChainPositions::HighCut>();

    updateCutFilter(leftHighCut, highCutCoefficients, chainSettings.highCutSlope);
    updateCutFilter(righttHighCut, highCutCoefficients, chainSettings.highCutSlope);
}
...
// Implementação dos Filtos: Low, High, Parametric/Peak
// Frquencias de corte: Controle de frequencias/Slope
// Parametros de frequencia: Controle, Ganho, Fator Gualidade; 
juce::AudioProcessorValueTreeState::ParameterLayout EQAudioProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>("Peak Quality",
        ...
        1.f)); layout.add(std::make_unique<juce::AudioParameterFloat>("Peak Quality", "Peak Quality", juce::NormalisableRange<float>(0.1f, 10.f, 0.05f, 1.f), 1.f));
